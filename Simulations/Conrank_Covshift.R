...
# define outputs
Err_SCP <- matrix(NA, nrow = nSD, ncol = nReps)
Err_tSCP <- matrix(NA, nrow = nSD, ncol = nReps)
Err_cox <- matrix(NA, nrow = nSD, ncol = nReps)

# AMI
AMI_SCP <- matrix(NA, nrow = nSD, ncol = nReps)
AMI_tSCP <- matrix(NA, nrow = nSD, ncol = nReps)
AMI_cox <- matrix(NA, nrow = nSD, ncol = nReps)

# max variant of NMI
NMI_M_SCP <- matrix(NA, nrow = nSD, ncol = nReps)
NMI_M_tSCP <- matrix(NA, nrow = nSD, ncol = nReps)
NMI_M_cox <- matrix(NA, nrow = nSD, ncol = nReps)

# sum variant of NMI
NMI_S_SCP <- matrix(NA, nrow = nSD, ncol = nReps)
NMI_S_tSCP <- matrix(NA, nrow = nSD, ncol = nReps)
NMI_S_cox <- matrix(NA, nrow = nSD, ncol = nReps)

# ARI
ARI_SCP <- matrix(NA, nrow = nSD, ncol = nReps)
ARI_tSCP <- matrix(NA, nrow = nSD, ncol = nReps)
ARI_cox <- matrix(NA, nrow = nSD, ncol = nReps)

# Purity
Purity_SCP <- matrix(NA, nrow = nSD, ncol = nReps)
Purity_tSCP <- matrix(NA, nrow = nSD, ncol = nReps)
Purity_cox <- matrix(NA, nrow = nSD, ncol = nReps)
...
k_SCP <- matrix(NA, nrow = nSD, ncol = nReps)
k_tSCP <- matrix(NA, nrow = nSD, ncol = nReps)
k_cox <- matrix(NA, nrow = nSD, ncol = nReps)

# record running time 
time_SCP <- matrix(NA, nrow = nSD, ncol = nReps)
time_tSCP <- matrix(NA, nrow = nSD, ncol = nReps)
time_cox <- matrix(NA, nrow = nSD, ncol = nReps)
...
for (i in 1:nSD) {
  ...
  ##### Source data #######
  #########################
  # We assume beta is different, but the same order
  beta_S <- log(rank(beta_T, ties.method = "first"))
  # Fix the shift
  sd <- sdSeq[i]
  X_S <- matrix(runif(n_S*p, -1/2, 1/2), nrow = n_S, ncol = p)+
         matrix(rnorm(n_S*p, 0, sd), nrow = n_S, ncol = p)
  ...
  for (j in 1:nReps) {
    ...
    save.image(file = "Conrank_Covshift2_n200.RData")
  }
