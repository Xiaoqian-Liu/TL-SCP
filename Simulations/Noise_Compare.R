...
# define outputs
Err_Lasso <- matrix(NA, nrow = nSig, ncol = nReps)
Err_SCAD <- matrix(NA, nrow = nSig, ncol = nReps)
Err_MCP <- matrix(NA, nrow = nSig, ncol = nReps)
Err_Dist <- matrix(NA, nrow = nSig, ncol = nReps)
Err_cox <- matrix(NA, nrow = nSig, ncol = nReps)



# AMI
AMI_Lasso <- matrix(NA, nrow = nSig, ncol = nReps)
AMI_SCAD <- matrix(NA, nrow = nSig, ncol = nReps)
AMI_MCP <- matrix(NA, nrow = nSig, ncol = nReps)
AMI_Dist <- matrix(NA, nrow = nSig, ncol = nReps)
AMI_cox <- matrix(NA, nrow = nSig, ncol = nReps)


# max variant of NMI
NMI_M_Lasso <- matrix(NA, nrow = nSig, ncol = nReps)
NMI_M_SCAD <- matrix(NA, nrow = nSig, ncol = nReps)
NMI_M_MCP <- matrix(NA, nrow = nSig, ncol = nReps)
NMI_M_Dist <- matrix(NA, nrow = nSig, ncol = nReps)
NMI_M_cox <- matrix(NA, nrow = nSig, ncol = nReps)
...
k_Dist <- matrix(NA, nrow = nSig, ncol = nReps)

# record running time 
time_Lasso <- matrix(NA, nrow = nSig, ncol = nReps)
time_SCAD <- matrix(NA, nrow = nSig, ncol = nReps)
time_MCP <- matrix(NA, nrow = nSig, ncol = nReps)
time_Dist <- matrix(NA, nrow = nSig, ncol = nReps)
time_cox <- matrix(NA, nrow = nSig, ncol = nReps)
...
for (i in 1:nSig) {
  ...
  for (j in 1:nReps) {
    ...
    # get the status by fixing the censor_rate
    censor_rate <- 0.3
    status <- rep(1, n)
    ...
    save.image(file = "SCP_noise.RData")
  }
  
  cat('case', i, 'finish \n')
  
  Beta_Lasso[[i]] <- beta_Lasso
  Beta_AP_Lasso[[i]] <- beta_AP_lasso
